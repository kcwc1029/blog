...
##### 2.7.5.1. Fence Register(界限占存器)
- Fence Register(界限占存器) 是用來儲存OS的大小
![Image](https://cdn.discordapp.com/attachments/1286741860538122281/1335969610595565648/image.jpg?ex=67a21a28&is=67a0c8a8&hm=f2374c017f9f148e5c8737b848c7f425f90fb4ad82245c66f6a5edd99911f628&)

- 所以，需要將【設定Fence Register】相關的指令，設為特權指令。
- 缺點：當OS所在的區域動態改變，會造成user program的期使位置有所改變，所有user program都需要重新定址 => 需要dynamic binding支援
- 為何OS所在的區域動態改變?
	- OS通常有許多錯誤處理程序，但不會一次就全部載入到記憶體中。
	- 當有一個錯誤產生，當OS在記憶體中找不到對應處理程序時，就會利用dynamic loading的方式，從disk中載入對應的處理程序，所以OS大小會改變。
##### 2.7.5.2. 讓Monitor Area與User Area往相反方向增長
- Monitor Area:由低往高擺。
- User Area:由高往低擺。
![upgit_20250203_1738591009.png](https://raw.githubusercontent.com/kcwc1029/obsidian-upgit-image/main/2025/02/upgit_20250203_1738591009.png)

##### 2.7.5.3. 各個user area的保護
- 每一個user area都要有base register跟limit register
- base register用來記錄user program 的期使位置
- limit register用來記錄user program 的大小
- 對於user  program 要執行時，會檢查：
	- 存取位置是否小於base register(侵犯到上一個program位置)
	- 存取位置是否大於base register+limit register(侵犯到下一個program位置)
...
- System Call的類型可概分如下:
	...
	- 通信(Communications)
- user通常看不到system call的細節，都是透過in-line或run-time support system的方式為多數程式語言提供一個簡單介面。
![upgit_20250203_1738593111.png](https://raw.githubusercontent.com/kcwc1029/obsidian-upgit-image/main/2025/02/upgit_20250203_1738593111.png)
