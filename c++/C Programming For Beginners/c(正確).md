...
-   https://cygwin.com/
-   安裝 3 個套件：gcc-core, gdb(只能使用 9.2.1 版本), cmake
...
int main(void)
{
    ...
    printf("number 的位址: %p\n", &number); // 輸出 number 的記憶體位址
    printf("number 的值: %d\n", number);  // 輸出 number 的值

    pnumber = &number;                   // 將 number 的位址儲存在指標 pnumber 中

    printf("pnumber 的位址: %p\n", (void*)&pnumber); // 輸出指標 pnumber 本身的位址
    printf("pnumber 的大小: %zd bytes\n", sizeof(pnumber)); // 輸出指標 pnumber 的大小
    printf("pnumber 的值: %p\n", pnumber);   // 輸出 pnumber 的值，即 number 的位址
    printf("pnumber 指向的值: %d\n", *pnumber); // 輸出 pnumber 指向的記憶體位置中的值，即 number 的值
    return 0;
}
```

## 4. 動態記憶體分配 (Dynamic Memory Allocation)

-   malloc (記憶體分配)

    -   分配指定大小的記憶體，返回指向分配記憶體區塊的指標。
    -   分配的記憶體未初始化，內容為未定義。
...
-   calloc (連續記憶體分配並初始化)
    -   分配指定數量的連續記憶體區塊，並將分配的記憶體初始化為 0。
    -   使用兩個參數：分配的記憶體區塊數量和每個區塊的大小。
-   realloc (重新調整記憶體大小)
